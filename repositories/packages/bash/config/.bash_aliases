...
# @info Show a sorted list of file and directory sizes
# @group files
function files() {
    du --all --total --human-readable "$@" | sort --human-numeric-sort
}

# @info Show a detailed list of all files and directories
# @group files
...
# @info Register all projects in \`~/Projects\` with gita
# @group projects
function proj_sync() {
    gita add "${HOME}/Projects"/*/
}

# @info Show git status for all registered repos
# @group projects
