...
(
  ...
  printf "1\n4\n2\ny\ny" | ./install.sh 2>&1 | tee "./install.log"
  ...
  wget -qO- https://github.com/catppuccin/cursors/releases/download/v2.0.0/catppuccin-mocha-mauve-cursors.zip | bsdtar -xvf- -C "${icons_dir}" 2>&1 | tee "./install.log"
  
  if [ ! -L "${HOME}/.icons" ]; then
    ln -s "${icons_dir}" "${HOME}/.icons"
  fi

  plasma-apply-cursortheme "catppuccin-mocha-mauve-cursors" 2>&1 | tee "./install.log"
)
